...
import { ProfileAccountStatus } from "../types/profile"
...
export const checkEmail = async (email: string): Promise<Boolean> => {
    try {
        if (await ProfileModel.exists({ contact: { email } })) return false
        return true
    } catch (error) { throw new ErrorHandler("Erreur de connexion à la base de donnée, nous y travaillons!", 500, error) }
}
...
export const sendEmail = async (email: string): Promise<TSendEmail> => {
    try {
        let profile = await ProfileModel.findOne({ contact: { email } })
        if (profile) {
            if (profile.confirmation.accountStatus == ProfileAccountStatus.active) {
                let recoveryCode = Math.floor(Math.random() * 1000)
                profile.confirmation.recoveryCode = recoveryCode.toString()
                profile.confirmation.accountStatus = ProfileAccountStatus.pending
                await profile.save()
...
export const verifyEmail = async (email: string, code: string) => {
    try {
        const profile = await ProfileModel.findOne({ contact: { email } })
        if (profile) {
            if (profile.confirmation.recoveryCode == code) {
                profile.confirmation.recoveryCode = '0'
                profile.confirmation.accountStatus = ProfileAccountStatus.active
                await profile.save()
...
export const signup = async (profile: TSignup): Promise<String> => {
    ...
    const newProfile = new ProfileModel({
        ...
        confirmation: {
            ...
            accountStatus: ProfileAccountStatus.pending
        },
    })
    try {
...
export const signin = async (email: string, password: string): Promise<String> => {
    const profile = await ProfileModel.findOne({ contact: { email }, password: sha(password) })
