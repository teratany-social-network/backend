  ...
  try {
    ...
    if (profile) {
      ...
      throw new ErrorHandler(
        "Vous essayez de publier avec un compte qui n'est pas enregistré",
        403,
        new Error()
      );
  } catch (error) {
    if (error instanceof ErrorHandler) throw error;
      ...
      throw new ErrorHandler(
        "Erreur interne au serveur, nous y travaillons",
        500,
        error
      );
  }
...
export const getProfilePublication_ = async (
  profileId: String,
  ownId: string
): Promise<any[]> => {
  try {
...
export const getProfilePublication = async (
  profileId: string,
  ownId: string
): Promise<IPub[]> => {
  try {
...
export const getOnePublication = async (
...
export const updatePublication = async (
  ...
  await PublicationModel.findByIdAndUpdate(publicationId, { content }).catch(
    (error) => {
      throw new ErrorHandler("Erreur interne, nous y travaillons.", 500, error);
    }
  );
};
...
export const postComment = async (
  ...
  try {
    ...
    if (publication) {
      ...
      throw new ErrorHandler(`La publication n'existe pas`, 404, new Error());
  } catch (error) {
    if (error instanceof ErrorHandler) throw error;
    else
      throw new ErrorHandler(
        `Erreur côté serveur, nous y travaillons.`,
        500,
        error
      );
  }
...
export const removeComment = async (commentId: string) => {
  await CommentModel.findByIdAndDelete(commentId).catch((error) => {
    throw new ErrorHandler(
      `Erreur côté serveur, nous y travaillons`,
      500,
      error
...
export const removePublication = async (publicationId: string) => {
  await PublicationModel.findByIdAndDelete(publicationId).catch((error) => {
    throw new ErrorHandler(
      `Erreur côté serveur, nous y travaillons`,
      500,
      error
...
export const toggleReaction = async (
  ...
  try {
    ...
    if (error instanceof ErrorHandler) throw error;
      ...
      throw new ErrorHandler(
        "Une erreur s'est produite lors de la mise à jour des réactions de l'utilisateur pour la publication, nous y travaillons",
        500,
        error
      );
  }
...
export const getComments = async (publicationId: string): Promise<any[]> => {
  const publication = await PublicationModel.findById(publicationId).populate({
    path: "comments",
    populate: {
      path: "profile",
    ...
    },
    {
      $addFields: {
        publication: {
          ...
          isReacted: {
            $in: [
              new Types.ObjectId(ownId),
              "$followingPublications.reactions",
            ],
          },
        },
      },
